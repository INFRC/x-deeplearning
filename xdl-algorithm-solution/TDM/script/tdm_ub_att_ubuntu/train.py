...
def init():
    ...
    tree.load()
    print("JC Time "+"[init --> load tree] {}".format(time.time()-begin_time))
...
def train(is_training=True):
    ...
    if is_training:
        data_io = xdl.DataIO("tdm", file_type=file_type, fs_type=xdl.fs.hdfs,
                             namenode="hdfs://localhost:8020", enable_state=False)
    ...
    for i in xrange(1, feature_count + 1):
        ...
        eb = xdl.embedding(emb_name, batch["item_%s" % i], xdl.Normal(stddev=0.001), emb_dim, 1000000, emb_combiner, vtype="hash", feature_add_probability=feature_add_probability)
        with xdl.device('GPU'):
            eb_take = xdl.take_op(eb, batch["indicators"][0])
        eb_take.set_shape(eb.shape)
        emb.append(eb_take)
    #emb_name = "unit_id_expand_emb"
    unit_id_expand_emb = xdl.embedding(emb_name, batch["unit_id_expand"], xdl.Normal(stddev=0.001), emb_dim, 1000000, emb_combiner, vtype="hash", feature_add_probability=feature_add_probability)

    args = list()
    auxs = list()

    @xdl.mxnet_wrapper(is_training=is_training, device_type='gpu')
    def dnn_model_define(user_input, indicator, unit_id_emb, label, eb_dim, fea_groups, active_op='prelu', use_batch_norm=True):
        ...
        layer_data = []
        # start att
        att_user_input = mx.sym.reshape(user_input, (-1, total_group_length, eb_dim))
        att_node_input = mx.sym.reshape(unit_id_emb, (-1, 1, eb_dim))
        ...
        att_active_op = 'prelu'
        att_layer_arr = []
        att_layer1 = FullyConnected3D(3*eb_dim, 36, active_op=att_active_op, version="1", feature_count=feature_count)
        att_layer_arr.append(att_layer1)
        att_layer2 = FullyConnected3D(36, 1, active_op=att_active_op, version="2", feature_count=feature_count)
        ...
        ph_label_click = mx.sym.reshape(ph_label_click, shape=(-1, ))
        #ph_label_click = mx.sym.expand_dims(ph_label_click, axis=1)
    
        #prop = mx.symbol.SoftmaxOutput(data=dout, label=label, grad_scale=1.0, use_ignore=True, normalization='valid')
        ...
        ph_label_sum = mx.sym.reshape(ph_label_sum, shape=(-1, 1))
        #ph_label_sum = mx.sym.expand_dims(ph_label_sum, axis=1)
        origin_loss = mx.sym.broadcast_mul(origin_loss, ph_label_sum)
        loss = - mx.symbol.sum(origin_loss) / mx.sym.sum(ph_label_sum)
        return prop, loss
    re = dnn_model_define(emb, batch["indicators"][0], unit_id_expand_emb, batch["label"], emb_dim, '20,20,10,10,2,2,2,1,1,1')
    prop = re[0]
    loss = re[1]
    ...
    print("JC Time "+"[dnn train] {}".format(time.time()-begin_time2))
    if is_training:
        if xdl.get_task_index() == 0:
